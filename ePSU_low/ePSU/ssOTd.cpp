...
void silentRecv(u32 numOTs, Socket &chl, PRNG& prng, BitVector &choices, AlignedVector<block> &rMsgs, u32 numThreads ){
    ...
    coproto::sync_wait(receiver.genBaseOts(prng, chl)); // Generate base OTs.

    coproto::sync_wait(receiver.silentReceive(choices, rMsgs, prng, chl, OTType::Random)); // Run silent OT.
...
void ssOTd(u32 idx, u32 numElements, std::vector<block> permutedX0, BitVector &choices, std::vector<block> &Out, Socket &chl, u32 numThreads ){ 
    ...
    if(idx == 0){
        ...
        for(u32 i = 0; i < numElements; i++){
            vecOTP_out[i] = OToutputs[i] ^ permutedX0[i];
        }
