...
# =================setting=================
BIN="$(cd "$(dirname "$0")" && pwd)/build/psPMT"
LOG_DIR="./logs_psPMT_$(date +%Y%m%d_%H%M%S)"
NN_LIST=(14 16 18 20 22)
NT=1
NS_NAME="pspmt_bench_ns"
...
for config in "${NET_CONFIGS[@]}"; do
    ...
    TC_CMD="tc qdisc add dev lo root netem delay $DELAY rate $BANDWIDTH $EXTRA_ARGS"
    ip netns exec $NS_NAME $TC_CMD
    ...
    for NN in "${NN_LIST[@]}"; do
        ...
        ip netns exec $NS_NAME sudo -u $REAL_USER $BIN -r 0 -nt $NT -nn "$NN" >> "$LOG" 2>&1 &
        P0_PID=$!

        sleep 2

        # Role 1 starts, connects to Role 0
        ip netns exec $NS_NAME sudo -u $REAL_USER $BIN -r 1 -nt $NT -nn "$NN" >> "$LOG" 2>&1 &
        P1_PID=$!
